...
void CVoiceChatClient::OnPacketIncoming(ENetEvent& event)
{
	...
	switch (id)
	{
		case 244:
		{
			...
			SAMP_DUMP("voice_test", "speaking -> playerid: %d", playerid);

		/*	if(pNetGame && pNetGame->GetPlayerPool() && pNetGame->GetPlayerPool()->GetSlotState(playerid)) 
			{
				if(pNetGame->GetNotificationPool()->IsPlayerAlreadyAdded(playerid)) {
					break;
				}
				ImGuiIO &io = ImGui::GetIO();
				char bufferNotify[0xFF+1] = { '\0' };
				sprintf(bufferNotify, "[%d] %s", playerid, pNetGame->GetPlayerPool()->GetPlayerName(playerid));
				pNetGame->GetNotificationPool()->Add(0, 0, 500, ImVec2(pGUI->GetFontSize() * 4.5f, io.DisplaySize.y * 0.57f), bufferNotify, 42.0f, 0xFFFAFAFA, true, 0xFFFF0000, 2, true, ImVec2(64, 64), pNetGame->GetNotificationPool()->GetCachedIconTexture(0), false);
			}*/
...
{
	if (pChatWindow) pChatWindow->AddDebugMessage(OBFUSCATE("�� ��������� �� ���� ����"));
