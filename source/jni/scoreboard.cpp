...
void CScoreBoard::Draw()
{
	...
	char serverTitle[128];

	cp1251_to_utf8(serverTitle, pNetGame->m_szHostName, 128);

	std::string playersUTF8 = "Игроков";
	char players[128];

	cp1251_to_utf8(players, playersUTF8.c_str());

	sprintf(szServerAddress, "%s | %s: %d", serverTitle, players, m_pPlayerCount);
