...
void CSettings::Save(int iIgnoreCategory)
{
	...
	if (iIgnoreCategory != 1)
	{
		...
		ini_table_create_entry_as_int(config, "gui", "timeinhud", m_Settings.iTimeInHud);
		ini_table_create_entry_as_int(config, "gui", "onlineinhud", m_Settings.iOnlineInHud);
		ini_table_create_entry_as_int(config, "gui", "newdialogs", m_Settings.iNewDialogs);
		ini_table_create_entry_as_int(config, "gui", "newhud", m_Settings.iNewHud);
		ini_table_create_entry_as_int(config, "gui", "chathidden", m_Settings.iChatHidden);
		ini_table_create_entry_as_int(config, "gui", "fx", m_Settings.iFX);

                                   // ini_table_create_entry_as_int(config, "gui", "version", m_Settings.iVersion);
		...
		ini_table_create_entry_as_int(config, "gui", "killlist", m_Settings.iKillList);
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
	...
	m_Settings.fButtonPassengerCycleX = ini_table_get_entry_as_float(config, "gui", "PassengerCycleX", 100.0f);
	m_Settings.fButtonPassengerCycleY = ini_table_get_entry_as_float(config, "gui", "PassengerCycleY", 500.0f);
	m_Settings.fButtonPassengerCycleSize = ini_table_get_entry_as_float(config, "gui", "PassengerCycleSize", 128.0f);
	...
	m_Settings.iTimeInHud = ini_table_get_entry_as_int(config, "gui", "timeinhud", 1);
	m_Settings.iOnlineInHud = ini_table_get_entry_as_int(config, "gui", "onlineinhud", 1);
	m_Settings.iNewDialogs = ini_table_get_entry_as_int(config, "gui", "newdialogs", 1);
	m_Settings.iNewHud = ini_table_get_entry_as_int(config, "gui", "newhud", 1);
	m_Settings.iChatHidden = ini_table_get_entry_as_int(config, "gui", "chathidden", 1);
	m_Settings.iFX = ini_table_get_entry_as_int(config, "gui", "fx", 1);
	...
	m_Settings.iKillList = ini_table_get_entry_as_int(config, OBFUSCATE("gui"), OBFUSCATE("killlist"), 1);
	for (int i = 0; i < E_HUD_ELEMENT::HUD_SIZE; i++)
	{
		char buff[30];
		...
		else CAdjustableHudColors::SetHudColorFromRGBA((E_HUD_ELEMENT)i, -1, -1, -1, -1);
	}
