...
bool CTextDrawPool::IsSomeoneActiveOnScreen() {
...
{
	if(!ARRAY_VALIDATE(m_pTextDraw, wText)) {
		SAMP_DUMP("TextDrawPool", "Warning: textdraw %u has invalid ID (CTextDrawPool::New)", wText);
		return NULL;
	}
...
{
	if(!ARRAY_VALIDATE(m_pTextDraw, wText)) {
		SAMP_DUMP("TextDrawPool", "Warning: textdraw %u has invalid ID (CTextDrawPool::Delete)", wText);
		return;
	}
