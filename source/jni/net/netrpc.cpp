...
void InitGame(RPCParameters *rpcParams)
{
	...
	if (!pVoice)
	{
	 	...
	 	if (g_IsVoiceServer()) {
	 		pVoice->Connect(VOICE_SRV_IP, VOICE_SRV_PORT);
...
void DialogWindow(RPCParameters *rpcParams)
{
	...
	if(pDialogWindow->m_utf8Button1[0] == '\0' && pDialogWindow->m_utf8Button2[0] == '\0')
		return;

	if (pSettings->GetReadOnly().iNewDialogs)
	{
		g_pJavaWrapper->ShowDialog(pDialogWindow->m_wDialogID, byteDialogStyle, title, info, button1, button2);
	}
