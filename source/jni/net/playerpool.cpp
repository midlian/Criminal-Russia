...
CPlayerPool::CPlayerPool()
{
	...
	for(PLAYERID playerId = 0; playerId < ARRAY_SIZE(m_pPlayers); playerId++)
	{
...
PLAYERID CPlayerPool::GetCount()
{
	...
	for (PLAYERID p = 0; p < ARRAY_SIZE(m_pPlayers); p++) {
...
{
	if(!ARRAY_VALIDATE(m_pPlayers, playerId)) {
		SAMP_DUMP("PlayerPool", "Warning: player %u has invalid ID (CPlayerPool::New)", playerId);
		return false;
	}
...
{
	if(!ARRAY_VALIDATE(m_pPlayers, playerId)) {
		SAMP_DUMP("PlayerPool", "Warning: player %u has invalid ID (CPlayerPool::Delete)", playerId);
		return false;
	}
	...
	if(m_pLocalPlayer) {
		if(m_pLocalPlayer->IsSpectating() && m_pLocalPlayer->m_SpectateID == playerId) {
...
{
	for(int i = 0; i < ARRAY_SIZE(m_pPlayers); i++)
	{
