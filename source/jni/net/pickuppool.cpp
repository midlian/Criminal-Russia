...
CPickupPool::CPickupPool()
{
	...
	m_iPickupCount = 0;
	for(int i=0; i<ARRAY_SIZE(m_Pickups); i++)
	{
		m_dwHnd[i] = 0;
		m_iTimer[i] = 0;
		m_iPickupID[i] = -1;
...
{
	if(m_iPickupCount >= MAX_PICKUPS || iPickup < 0 || iPickup >= MAX_PICKUPS)
		return;

	if (!ARRAY_VALIDATE(m_Pickups, iPickup)) {
		SAMP_DUMP("PickupPool", "Warning: pickup %u has invalid ID (CPickupPool::New)", iPickup);
		return;
	}
...
{
	if(m_iPickupCount <= 0 || iPickup < 0 || iPickup >= MAX_PICKUPS)
		return;

	if (!ARRAY_VALIDATE(m_dwHnd, iPickup)) {
		SAMP_DUMP("PickupPool", "Warning: pickup %u has invalid ID (CPickupPool::Destroy)", iPickup);
		return;
	}
...
void CPickupPool::PickedUp(int iPickup)
{
	...
	if (!ARRAY_VALIDATE(m_dwHnd, index)) {
		SAMP_DUMP("PickupPool", "Warning: pickup %u has invalid ID (CPickupPool::PickedUp)", iPickup);
		return;
	}
	...
	if(m_dwHnd[index] != 0 /*&& m_iTimer[index] == 0*/)
	{
		...
		Log(OBFUSCATE("CPickupPool::PickedUp RPC"), index);
