...
std::string jstring2string(JNIEnv *env, jstring jStr) {
	...
	const jmethodID getBytes = env->GetMethodID(env->GetObjectClass(jStr), "getBytes", "(Ljava/lang/String;)[B");
	const auto strJBytes = (jbyteArray) env->CallObjectMethod(jStr, getBytes, env->NewStringUTF("windows-1251"));

	auto length = (size_t) env->GetArrayLength(strJBytes);
	jbyte* pBytes = env->GetByteArrayElements(strJBytes, nullptr);
