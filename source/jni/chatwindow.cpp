...
{
	void* handle;
	handle = dlopen("/data/data/criminal.russia/lib/libbass.so", 3);
...
bool ProcessVoiceCommands(const char* str)
{
	...
	// if (strcmp(str, "/g1") == 0)
	// {
	//	g_pJavaWrapper->CallLauncherActivity(1230);
	//	return true;
	// }
	// if (strcmp(str, "/g2") == 0)
	// {
	// 	g_pJavaWrapper->CallLauncherActivity(1231);
	// 	return true;
	// }
	// if (strcmp(str, "/z1") == 0)
	// {
	...
	// {
       	...
       	//	 {
        	//		pChatWindow->AddDebugMessage("[Switchhud]: Use: /switchhud [id]");
        	//		return true;
	//	}
	//	g_pJavaWrapper->SetHudLogo(id);
	// }
	if (strcmp(str, "/testbinder") == 0)
	{
		CBinder::Toggle();
	...
	if (strstr(str, "/tab") || strstr(str, "/online"))
	{
		pScoreBoard->Toggle();
		return true;
	}

	if (strstr(str, "/hudeditor") || strstr(str, "/hud") || strstr(str, "/client"))
	{
		g_pJavaWrapper->ShowClientSettings();
		return true;
	}

	if (strstr(str, "/fpsinfo") || strstr(str, "/fps"))
	{
	...
	if(strstr(str, "/ntf")) {
		// NotificationType type, NotificationId id, NotificationDuration duration, NotificationPosOnScreen vecPos, const char* text, float fFontSize, ImU32 textColor, bool bTextHasOutline, ImU32 textOutlineColor, int textOutlineOffset, bool bHasIcon, NotificationIconScaleOnScreen vecIconScale, uintptr_t pIconPointer
		// notifies[0] = new Notification(0, 0, 5555, ImVec2(555, 555), "test notify", 42.0f, 0xFFFAFAFA, true, 0xFFFF0000, 2, true, ImVec2(64, 64), LoadTextureFromDB("samp", "voice_button"));

		//NotificationType type, NotificationId id, NotificationDuration duration, NotificationPosOnScreen vecPos, const char* text, float fFontSize, ImU32 textColor, bool bTextHasOutline, ImU32 textOutlineColor, int textOutlineOffset, bool bHasIcon, NotificationIconScaleOnScreen vecIconScale, uintptr_t pIconPointer, bool bRecreate
		pNetGame->GetNotificationPool()->Add(0, 0, 5000, ImVec2(555, 555), "test notify from pool", 42.0f, 0xFFFAFAFA, true, 0xFFFF0000, 2, true, ImVec2(64, 64), pNetGame->GetNotificationPool()->GetCachedIconTexture(0), false);
	}
                  if (strstr(str, "/testvoice"))
	{
		g_bShowVoiceList =true;
		toggleVoiceStuff();
		return true;
	}
	if (strstr(str, "/vlist"))
	...
	if (strstr(str, "/vmute"))
	{
		...
		pChatWindow->AddDebugMessage("�� ��������� ������ ID: %d", id);
		return true;
	}
...
bool CChatWindow::OnTouchEvent(int type, bool multi, int x, int y)
{
	...
	switch (type)
	{
		case TOUCH_PUSH:
				...
				y >= m_fChatPosY && y <= m_fChatPosY + m_fChatSizeY)
			{
				...
				bWannaOpenChat = true;
				// g_pJavaWrapper->CallLauncherActivityWithSpecialValue(1223, pKeyBoard->IsOpen() ? 1 : 0);
			}
			break;
	...
	/*if(pKeyBoard->IsOpen())
 	{
 		g_pJavaWrapper->CallLauncherActivity(1223);
 	} else {
 		g_pJavaWrapper->CallLauncherActivity(1223);
 	}*/
	return true;
