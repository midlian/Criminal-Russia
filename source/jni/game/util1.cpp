...
eWidgetIds GetId(WIDGET_TYPE* pWidget)
{
	...
	if(pWidget == pWidgets.CrossHair) return TYPE_CROSSHAIR;
	if(pWidget == pWidgets.Shoot) return TYPE_SHOOT;
	if(pWidget == pWidgets.LeftShoot) return TYPE_LEFTSHOOT;
...
{
	if(!strcmp("accelerate", name))
		SetById(TYPE_ACCELERATE, pWidget);

	if(!strcmp("hud_car", name))
		SetById(TYPE_ENTERCAR, pWidget);

	if(!strcmp("hud_lockon", name))
		SetById(TYPE_CROSSHAIR, pWidget);

	if(!strcmp("shoot", name))
		SetById(TYPE_SHOOT, pWidget);

	if(!strcmp("leftshoot", name))
		SetById(TYPE_LEFTSHOOT, pWidget);
}
...
void SetById(int idWidget, WIDGET_TYPE* pWidget)
{
	switch (idWidget)
	{
		...
		case TYPE_ENTERCAR:
			...
			break;
		case TYPE_CROSSHAIR:
			pWidgets.CrossHair = pWidget;
			break;
		case TYPE_SHOOT:
			pWidgets.Shoot = pWidget;
			break;
		case TYPE_LEFTSHOOT:
			pWidgets.LeftShoot = pWidget;
			break;
		default:
			break;
	}
...
eWidgetState ProcessFixed(WIDGET_TYPE* pWidget)
{
	...
	switch (idWidget)
	{
		...
		case TYPE_ATTACK:
		{
			//uint8_t interiorId = 0;
			//if (pNetGame)
			//	interiorId = pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->GetInterior();
		...
		case TYPE_SPRINT:
		//	if(pGame->FindPlayerPed()->IsInVehicle() || pGUI->m_bCarryStuff)
		//		return STATE_FIXED;
			break;
		case TYPE_ACCELERATE:
