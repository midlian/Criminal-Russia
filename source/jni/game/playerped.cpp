	...
	if (pNetGame)
	{
		...
		if (pPlayerPool)
		{
			...
			if (pLocalPlayer)
			{
				...
				if (pPlayerPed)
					g_pJavaWrapper->UpdateHudIcon((int)pPlayerPed->GetCurrentWeaponSlot()->dwType);
			}
		}
	}
}
...
void CPlayerPed::ProcessAttach()
{
	...
	if (m_pPed->entity.vtable == (SA_ADDR(0x5C7358)))
		return;
...
{
	if (!m_pPed || !m_dwGTAId || !GamePool_Ped_GetAt((int)m_dwGTAId)) {
		return;
                  }

	m_iCurrentWeaponPos++;

	if (m_iCurrentWeaponPos == 13)
		m_iCurrentWeaponPos = 0;

	if (m_iCurrentWeaponPos == 0)
		SetArmedWeapon(0);
	else
	{
		auto weaponSlot = m_pPed->WeaponSlots[m_iCurrentWeaponPos];

		if (weaponSlot.dwAmmo == 0)
		{
			SwitchWeapon();
			return;
		}
		else SetArmedWeapon((int)weaponSlot.dwType);
	}

	g_pJavaWrapper->UpdateHudIcon((int)GetCurrentWeaponSlot()->dwType);
}