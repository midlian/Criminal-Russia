	...
	for (auto& i : m_Plates)
	{
		...
		if (pPlate)
		{
			if (pNetGame->GetVehiclePool())
			{
				...
				{
					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture(OBFUSCATE("remap_plate"), pPlate); // BR && BRP

 					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("plate_remap", pPlate); // BR OLD
					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("platecharset", pPlate); // SAMP

					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("platem_remap", pPlate); // BR

                                                                                          pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("nomer", pPlate); // BR
                                                                                          pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("nomera", pPlate); // BR                                                                                      
				}
			}
		}
	}
