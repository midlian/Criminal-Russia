			...
			switch (type)
			{
				...
				case TOUCH_PUSH:
						if (m_pWidgets[id]->IsXYInRect(x, y)) {
							...
							//pChatWindow->AddDebugMessage("Wid TOUCH_PUSH");
						}
					...
					case TOUCH_POP:
						if (m_pWidgets[id]->IsXYInRect(x, y))
						{
							//pChatWindow->AddDebugMessage("TOUCH_POP");
							m_pWidgets[id]->SetState(2);
							if(id == WIDGET_MICROPHONE) {
								//m_pWidgets[id]->SetState(10);
								//pChatWindow->AddDebugMessage("Mirk");
								m_pWidgets[id]->StatusMicro(false);
							}
							return;
						}
