...
void RenderSplash()
{
	...
	Draw(&rect, ImColor(0x8B, 0x00, 0x00, 255));
}
...
{
	RenderSplash();
                  g_pJavaWrapper->SetLoadingText(0); // SHOW

	if (!pGUI) return;
            
	ImGui_ImplRenderWare_NewFrame();
	ImGui::NewFrame();
                  /* ТУТ ДОЛЖЕН БЫЛ БЫТЬ ТЕКСТ НО ЧТО-ТО ПОШЛО НЕ ТАК */
	ImGui::EndFrame();
	ImGui::Render();
