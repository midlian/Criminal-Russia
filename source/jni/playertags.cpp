...
CPlayerTags::CPlayerTags()
{
	...
	m_pVoice_icon = (RwTexture*)LoadTextureFromDB(OBFUSCATE("samp"), OBFUSCATE("voice_button2")); //micro_icon
...
void CPlayerTags::Render()
{
	...
	char szNickBuf[50];
	int iVoiceCounter = 0;
	ImVec2 basePos = ImVec2(pGUI->ScaleX(140.0f), pGUI->ScaleY(600.0f));
	if(pNetGame && pNetGame->m_bShowPlayerTags)
	{
		...
		for (int i = 0; i <= 6; i++)
		{
			...
			iconPos.x += ImGui::CalcTextSize(szNickBuf).x + (pGUI->GetFontSize() * 0.4f);
			if (m_pVoice_icon)
			{
				ImVec2 a = ImVec2(iconPos.x, iconPos.y);
				ImVec2 b = ImVec2(iconPos.x + pGUI->GetFontSize(), iconPos.y + pGUI->GetFontSize());
				ImGui::GetOverlayDrawList()->AddImage((ImTextureID)m_pVoice_icon->raster, b, a);
			}
