...
CKeyBoard::CKeyBoard()
{
	...
	m_bEnable = false;
	m_iLayout = LAYOUT_RUS;
	m_iCase = LOWER_CASE;
	m_iPushedKey = -1;
	m_iCurrentCursorPosition = 0;
...
#else
	std::string notificationUTF8 = "Щоб вставити текст, натисніть двічі на поле введення";
#endif
...
void CKeyBoard::Open(keyboard_callback* handler, bool bHiden)
{
	...
	m_bEnable = true;
	m_bInputFlag = bHiden;
	m_iCurrentCursorPosition = strlen(m_utf8Input);
...
void CKeyBoard::Close()
{
	...
	// g_pJavaWrapper->CallLauncherActivityWithSpecialValue(1227, this->IsOpen());
}
#include "util/CJavaWrapper.h"
...
void CKeyBoard::InitRU()
{
	...
	key.symPos = ImVec2(curPos.x + defWidth * 0.4, curPos.y + m_fKeySizeY * 0.2);
	key.width = defWidth;
	key.code[LOWER_CASE] = 'ї';
	key.code[UPPER_CASE] = 'Ї';
	cp1251_to_utf8(key.name[LOWER_CASE], "ї");
	cp1251_to_utf8(key.name[UPPER_CASE], "Ї");
	key.id++;
	it = row->begin();
	row->insert(it, key);
	curPos.x += key.width;
	...
	key.symPos = ImVec2(curPos.x + defWidth * 0.4, curPos.y + m_fKeySizeY * 0.2);
	key.width = defWidth;
	key.code[LOWER_CASE] = 'і';
	key.code[UPPER_CASE] = 'Ы';
	cp1251_to_utf8(key.name[LOWER_CASE], "і");
	cp1251_to_utf8(key.name[UPPER_CASE], "I");
	key.id++;
	it = row->begin();
	row->insert(it, key);
	...
	key.symPos = ImVec2(curPos.x + defWidth * 0.4, curPos.y + m_fKeySizeY * 0.2);
	key.width = defWidth;
	key.code[LOWER_CASE] = 'є';
	key.code[UPPER_CASE] = 'Є';
	cp1251_to_utf8(key.name[LOWER_CASE], "є");
	cp1251_to_utf8(key.name[UPPER_CASE], "Є");
	key.id++;
	it = row->begin();
	row->insert(it, key);
	curPos.x += key.width;
